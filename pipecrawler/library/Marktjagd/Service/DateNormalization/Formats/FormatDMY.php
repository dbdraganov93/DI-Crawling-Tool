...
class FormatDMY extends AbstractFormat implements DateNormalizationFormat
{
    protected const SUPPORTED_FORMATS = [
        'dmY',
        'dmY H:i:s',
    ];
    protected const REGEX_PATTERN = '#^(?<day>\d{2})(?<month>\d{2})(?<year>\d{2,3,4})*(?: *(?<hour>\d{1,2})?(?:\:(?<minute>\d{1,2})?(?:\:(?<second>\d{1,2}))*)*)*$#';
