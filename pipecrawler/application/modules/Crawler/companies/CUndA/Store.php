...
/*
 * Store Crawler für C&A (ID: 121)
 */

class Crawler_Company_CUndA_Store extends Crawler_Generic_Company {

    public function crawl($companyId) {
        $baseUrl = 'https://www.c-and-a.com/';
        $searchUrl = $baseUrl . 'stores/de-de/';
        ...
        foreach ($cityMatches[1] as $singleCity) {
            ...
            foreach ($storeMatches[1] as $singleStore) {
                ...
                if (preg_match($pattern, $singleStore, $websiteMatch)) {
                    ...
                    $pattern = '#Unser\s*Service\s*in\s*der\s*Filiale<[^>]*>(.+?)</ul#i';
                ...
                if (preg_match('#^(ü)#', $eStore->getCity())) {
                    $eStore->setCity(preg_replace('#^(ü)#', 'Ü', $eStore->getCity()));
                }
