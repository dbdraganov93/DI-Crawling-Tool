...
/**
 * Store Crawler für Spar AT (ID: 72819)
 */
class Crawler_Company_SparAt_Store extends Crawler_Generic_Company
{
    public function crawl($companyId)
    {
        $baseUrl = 'https://www.spar.at';
        $searchUrl = $baseUrl . '/standorte/_jcr_content.stores.v2.html?filter=SPAR';
        ...
        foreach ($stores as $singleJStore) {
            ...
            foreach ($singleJStore->shopHours as $time) {
                ...
                foreach ($day as $singleDay) {
                    ...
                    $fromMinutes = $time->openingHours->from1->minute;
                    $toMinutes = $time->openingHours->to1->minute;

                    // fix irregularities on API
                    if ($fromMinutes == 0) {
                        $fromMinutes = '00';
                    }
                    if ($toMinutes == 0) {
                    ...
                    $openingHours[] = $singleDay . ' ' . $time->openingHours->from1->hourOfDay . ':' . $fromMinutes
                        . ' - ' . $time->openingHours->to1->hourOfDay . ":" . $toMinutes;
                }
            }
            ...
            // if invalid Store
            if ($singleJStore->city == 'Maria Ellend' && $singleJStore->zipCode == '2402') {
                $eStore->setLatitude('48.1093874')
                    ->setLongitude('16.6806496');
            }
