...
`PdfDownloaderService` now checks the HTTP status code when retrieving a PDF and
only proceeds with annotation and S3 upload if the download succeeds. Errors are
logged to help troubleshoot network issues.
Run the server
```
symfony serve -d
```
