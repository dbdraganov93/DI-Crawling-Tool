<?php
/**
 * Store Crawler für Ernsting's Family AT (ID: 72601)
 */

class Crawler_Company_ErnstingsAt_Store extends Crawler_Generic_Company
{
    ...
    public function crawl($companyId)
    {
        ...
        // Same folder as DE
        $localPath = $sFtp->connect('22133', TRUE);
        ...
        foreach ($storeData as $singleStore) {
            // Filter AT
            if (!preg_match('#AT#', $singleStore[self::STORE_LAND])) {
                continue;
            }
            ...
            // Exceptions for geolocation errors
            if($singleStore[self::STORE_PLZ] == 2460) {
                $singleStore[self::STORE_STREET] = "Eco-Plus-Park 4. Straße, 6";
            } elseif($singleStore[self::STORE_PLZ] == 4150) {
                $singleStore[self::STORE_CITY]   = "Rohrbach-Berg";
                $singleStore[self::STORE_STREET] = "Wirtschaftszeile, 2-4";
            }
