...
class Crawler_Company_Kik_GenerateArticleList extends Crawler_Generic_Company
{

    # modify these as needed
    private string $countryCode;
    private const articleFile = 'Artikelliste_';
    private const brochureFile = 'Artikelliste.pdf';
    private const ftpPath = 'OP KW 26 2022/Fotomaterial';
    private const title = 'KW 26';

    # array to get currency symbols and the correct Excel columns
    private array $aCurrency = [
        'AT' => ' €',
        'BG' => ' лв.',
        'CZ' => ' Kč',
        'DE' => ' €',
    ...
    public function crawl($companyId)
    {
        ...
        foreach ($this->aCurrency as $countryCode => $currency) {
            $this->countryCode = $countryCode;
            $localArticleFile = $this::tmpFilePath . $this::articleFile . $countryCode . '.xlsx';
            ...
            exec('rm -r ' . APPLICATION_PATH . '/../public/files/ftp/340');
        }
        return TRUE;
    }
    ...
    public function buildTemplate(Marktjagd_Service_Output_Pdf $sPdf, string $localBrochurePath): array
    {
        ...
        # build the placeholder list - we plan with 19 page template (á 9 articles)
        $aProductPlaces = [];
        for ($i = 1; $i <= 19; $i++) {
            $aProductPlaces[] = [$i => []];
        }
        ...
        foreach ($aAnnots as $annotation) {
            ...
            if (!preg_match('#_(\d)+$#', $annotation->url)) {
                continue;
            }
    ...
    public function getArticlesToSet(Marktjagd_Service_Input_PhpSpreadsheet $sPss, string $localArticleFile, string $localImagePath): array
    {
        ...
        foreach ($aData as $rowNr => $singleRow) {
            if (!$singleRow['Bild URL']
                || !$singleRow['Produktname']
                || $singleRow['Preis aktuell'] == '#REF!'
                || $singleRow['Preis aktuell'] == '#N/A'
                || $singleRow['Produktname'] == '#N/A'
                || preg_match('#(dopplung|0)#i', $singleRow['Kategorie'])) {
                continue;
            }
        ...
        if (!function_exists('cmp')) {
            function cmp($a, $b)
            {
                return strcmp($a["order_nr"], $b["order_nr"]);
            }
        }

        usort($aArticles, "cmp");

        foreach ($aArticles as $index => $article) {
            if (preg_match('#NOS#', implode($article['long_text']))) {
                unset($aArticles[$index]);
                array_push($aArticles, $article);
            }
        }
        $aArticles = array_values($aArticles);
    ...
    public function fillTemplateWithArticles(array $aProductPlaces, array $aArticlesToSet, string $tmpFilePath): array
    {
        ...
        foreach ($aProductPlaces as $page) {
            foreach ($page as $productSlot) {
                ...
                $pageElements['heading'][$productSlot->page] = [
                    'page' => $productSlot->page,
                    'startX' => 60.0,
                    'startY' => 775.0,
                    'type' => 'text',
                    'contents' => $this->countryCode . ' endgültige Artikelliste, ' . $this::title,
                    'font' => ['fontType' => $textFont, 'fontSize' => '22', 'fontColor' => '0|0|0']
                ];
                ...
                if (isset($productSlot->article['suggested_retail_price']) && $productSlot->article['suggested_retail_price'] != 0) {
                    $pageElements['suggestedprice'][] = [
                        'page' => $productSlot->page,
                        'startX' => $productSlot->rectangle->startX + 30.0 + $xOffset,
                    ...
                    $pageElements['redline'][] = [
                        'page' => $productSlot->page,
                        'startX' => $productSlot->rectangle->startX + 28.0 + $xOffset,
                        'endX' => $productSlot->rectangle->startX + 73.0 + $xOffset,
                ...
                foreach ($productSlot->article['colorcodes'] as $colorVariant) {
                    $pageElements['colorcode'][] = [
                        'page' => $productSlot->page,
                        'startX' => $productSlot->rectangle->startX + 15.0 + $xOffset,
                        'endX' => $productSlot->rectangle->startX + 15.0 + $size + $xOffset,
                        'startY' => $productSlot->rectangle->startY + 75.0 + $yOffset,
                        'endY' => $productSlot->rectangle->startY + 75.0 + $size + $yOffset,
                        'type' => 'image',
                        'path' => $colorVariant,
                        'scaling' => FALSE
                    ];
    ...
    public function addArticleToList($singleRow, array &$aArticles, string $imageField, string $localImagePath): void
    {
        ...
        $longText = 'Art-Nr.: ' . trim($singleRow['Artikelnummer']);
        if (!empty($singleRow['WGR']))
            $longText .= ' | WGR: ' . trim($singleRow['WGR']);
        if (!empty($singleRow['NOS']))
            $longText .= ' | NOS';
        if (!empty($singleRow['Modellbeispiel']))
            $longText .= ' | Modellbeispiel ';
        if (!empty($singleRow[$longTextCol]))
            $longText .= ' | ' . trim($singleRow[$longTextCol]);
        ...
        $aArticles[] = [
            ...
            'order_nr' => trim($singleRow['Artikelnummer']),
            ...
            'long_text' => $this->buildTextLayout($longText, 45),
