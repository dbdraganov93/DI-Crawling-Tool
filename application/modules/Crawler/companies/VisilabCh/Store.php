...
class Crawler_Company_VisilabCh_Store extends Crawler_Generic_Company {
        ...
        foreach ($storeMatches[0] as $key => $singleStore) {
            ...
            if (count($aAddress) == 2) {
                ...
                $eStore->setSubtitle($aAddress[0])
                        ->setStreetAndStreetNumber($aAddress[1], 'CH')
                        ->setZipcodeAndCity($aAddress[2]);

                if (preg_match('#CP#', $eStore->getStreet())
                    || preg_match('#case\s*postale#is', $eStore->getStreet())
                    || preg_match('#Postfach#', $eStore->getStreet())
                ) {
                    $eStore->setStreetAndStreetNumber($aAddress[0], 'CH');
                }
            ...
            $patternOpening = '#<div[^>]*class="block\-title"[^>]*>[^<]*ffnungszeiten[^<]*<\/div>\s*(.+?)\s*<\/table#';
