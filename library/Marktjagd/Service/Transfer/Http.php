...
class Marktjagd_Service_Transfer_Http
{
     ...
     * @return bool
     */
    public function getRemoteFile($url, $localPath, $dataName = '', $cipherSSL = false)
    {
        if (!is_array($url)) {
            $success = $this->_curlOneFile($url, $localPath, $dataName, $cipherSSL);
        } else {
     ...
     * @return bool
     */
    protected function _curlOneFile($url, $localPath, $dataName, $cipherSSL = false)
    {
    ...
    public function generatePublicHttpUrl($localFilePath)
    {
        ...
        if (!preg_match('#.*?(/files/(.*?))$#', $localFilePath, $match)) {
            $this->_logger->log(
        ...
        if ($s3Config->active) {
            ...
            $fileHandle = new Marktjagd_Service_Output_S3File('http/', Marktjagd_Collection_Api_Brochure::getUniqueName($localFilePath));
