...
class Marktjagd_Database_Entity_CrawlerConfig extends Marktjagd_Database_Entity_Abstract
{
    ...
    protected $_systemRunning;

    public const VALID_BACKEND_SYSTEMS = [
        ApiServiceProvider::API3,
        ApiServiceProvider::IPROTO,
    ];

    /**
     * Determines which Backend-API for a given crawler (e.g. APIv3 or iProto).
     */
    protected string $_backendSystem = ApiServiceProvider::IPROTO;

    public const BACKEND_ENV_PROD = 'production';
    public const BACKEND_ENV_STAGE = 'staging';
    public const BACKEND_ENV_TEST = 'testing';
    public const VALID_BACKEND_ENVS = [
        self::BACKEND_ENV_PROD,
        self::BACKEND_ENV_STAGE,
        self::BACKEND_ENV_TEST,
    ];

    /**
     * Determines which environment-specific API-configuration to use for a given crawler (see application/configs/apiClient.ini).
     */
    protected string $_backendEnv = self::BACKEND_ENV_PROD;
    ...
    /**
     * @param mixed $systemRunning
     */
    public function setSystemRunning($systemRunning)
    {
        $this->_systemRunning = $systemRunning;
        return $this;
    }
     ...
     * @return mixed
     */
    public function getSystemRunning()
    {
        return $this->_systemRunning;
    }
    ...
    public function setBackendSystem(string $system): self
    {
        if (!in_array($system, self::VALID_BACKEND_SYSTEMS)) throw new InvalidArgumentException("unexpected backend-system '$system'");
        $this->_backendSystem = $system;
        return $this;
    }

    public function getBackendSystem(): string
    {
        return $this->_backendSystem;
    }

    public function setBackendEnv(string $env): self
    {
        if (!in_array($env, self::VALID_BACKEND_ENVS)) throw new InvalidArgumentException("unexpected backend-env '$env'");
        $this->_backendEnv = $env;
        return $this;
    }

    public function getBackendEnv(): string
    {
        return $this->_backendEnv;
    }
}
